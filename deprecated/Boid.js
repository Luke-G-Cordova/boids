...
export default class Boid {
  ...
  canSee(vector) {
    ...
    oVec.sub(oPos);
    let tVecUp = this.velocity
      .clone()
      .addAngle(((this.eiboh / 2) * Math.PI) / 180);
    let tVecDown = this.velocity
      .clone()
      .addAngle((-(this.eiboh / 2) * Math.PI) / 180);
    oVec.addAngle(-this.velocity.getAngle());
    let oVecAng = oVec.getAngle();
    return (
      ((oVecAng >= 0 && oVecAng <= this.eibohR / 2) ||
        (oVecAng < 0 && oVecAng >= -this.eibohR / 2)) &&
      oVec.magnitude <= this.visibility
    );
  }
  rightOrLeft(vector) {
    let oVec = vector.clone();
    ...
    return {
      direction:
        oVec.magnitude > this.visibility ? 0 : oVecAng <= Math.PI ? -1 : 1,
