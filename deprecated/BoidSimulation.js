...
export default class BoidSimulation {
  ...
  loop(drawBoid, drawObstacles, angleChanger) {
    ...
    for (let i = this.flock.length - 1; i >= 0; i--) {
      ...
      if (count !== 0) {
        ...
        newAngle += this.alignment(
          this.flock[i],
          Math.atan2(avgDirSin, avgDirCos)
        );
      ...
      if (!!this.obstacles) {
        for (let j = this.obstacles.length - 1; j >= 0; j--) {
          ...
          newAngle += this.avoidobstacle(
            rol,
            this.obstacles[j],
            this.flock[i].visibility
          );
        }
      }
  ...
  seperation(rol, visibility) {
    ...
    return (
      this.seperationOffset *
      this.scale(visibility - rol.distance, 0, visibility, 0, Math.PI * 2) *
      rol.direction
    );
  }
  ...
  alignment(boid, heading) {
    ...
    let hold =
      diff > this.alignmentOffset ? 1 : diff < -this.alignmentOffset ? -1 : 0;
  ...
  cohesion(rol, visibility) {
    return (
      -this.cohesionOffset *
      this.scale(rol.distance, 0, visibility, 0, Math.PI * 2) *
      rol.direction
    );
  }
